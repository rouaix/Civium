...
fn run_network(cmd: NetworkCmd, data: &PathBuf) -> Result<()> {
    match cmd {
        NetworkCmd::Create { name, display_name, private } => {
            ...
            let network = Network::create(name.clone(), &admin_cid, display_name, Some(keypair.pub_key_b58()), false, None)
                .map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    match cmd {
        DirectoryCmd::Create { name, display_name } => {
            let keypair = store::load_identity(data)
                .map_err(|_| anyhow::anyhow!("no identity found — run `identity init` first"))?;
            let admin_cid = keypair.cid();
            let mut network = Network::create(name.clone(), &admin_cid, display_name, Some(keypair.pub_key_b58()), false, None)
                .map_err(|e| anyhow::anyhow!("{e}"))?;
